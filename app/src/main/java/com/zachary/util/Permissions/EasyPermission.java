 ...
 */
package com.zachary.util.Permissions;
...
public class EasyPermission {
    ...
    public static void requestPermissions(final Object object, String rationale,
        ...
        final List<String> deniedPermissions =
                PerUtils.findDeniedPermissions(PerUtils.getActivity(object), permissions);


        boolean shouldShowRationale = false;
        for (String perm : deniedPermissions) {
            shouldShowRationale =
                    shouldShowRationale || PerUtils.shouldShowRequestPermissionRationale(object, perm);
        }

        if (PerUtils.isEmpty(deniedPermissions)) {
            mCallBack.onPermissionGranted(requestCode, Arrays.asList(permissions));
        } else {
            ...
            if (shouldShowRationale) {
                Activity activity = PerUtils.getActivity(object);
                if (null == activity) {
                    return;
                }
    ...
    public static boolean checkDeniedPermissionsNeverAskAgain(final Object object, String rationale,
        ...
        for (String perm : deniedPerms) {
            ...
            if (!shouldShowRationale) {
                final Activity activity = PerUtils.getActivity(object);
                if (null == activity) {
                    return true;
                }

                alertDialog = new AlertDialog.Builder(activity).setMessage(rationale)
                        .setPositiveButton(positiveButton, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
